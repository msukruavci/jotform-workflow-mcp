...
SERVER_INSTRUCTIONS = """
When the user asks to create, design, or set up a workflow (even with brief, high-level requests), automatically call search_workflow_templates first to discover proven domain architectures from top-3 matching blueprints. Then call build_workflow_bulk directly in one shot with title, form_prompt or trigger_form_id, steps, and connections; build_workflow_bulk can create the AI trigger form, create the workflow, bind the trigger form, lay out the DAG, and write all steps/links. For an existing workflow, pass workflow_id to build_workflow_bulk. Include complete personalized email/task subjects, content/body, taskDescription, and {formField} placeholders in steps[].config during that initial bulk call; do not build basic placeholders and then loop through get_step_details/update_step. For standard approval/task/branch/email flows, use the core cached step schemas from the system prompt and skip exploratory list_step_types/get_step_schema calls; call get_step_schema only for specialized or unfamiliar step types from the user request or retrieved templates, and batch multiple unfamiliar types with get_step_schema(step_types=[...]). Inspect health/gaps after all writes are complete, fix required gaps first, then call show_workflow strictly once as the final presentation step.
